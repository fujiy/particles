...
pub const GRANULAR_ITERS: usize = 4;
...
const FAR_FIELD_EDGE_INSET_M: f32 = PARTICLE_RADIUS_M * 1.25;
    ...
    fn solve_contacts(
        ...
        for substep in 0..granular_substeps {
            ...
            for iter in 0..GRANULAR_ITERS {
                ...
                {
                    let _span =
                        tracing::info_span!("physics::granular_pair_contacts", substep, iter)
                            .entered();
                    for i in 0..n {
                        ...
                        for &j in &particles.neighbor_cache[i] {
                            ...
                            *lambda_n += delta_lambda_n;
                            ...
                            if let Some(scale) = granular_contact_friction_pair_scale(
                                ...
                                let mu_k = 0.5
                                    * (props_i.friction_dynamic + props_j.friction_dynamic)
                                    * scale;
                                ...
                                if tangent_len > 1e-6 {
                                    ...
                                    let next_lambda_t = (*lambda_t + delta_lambda_t)
                ...
                let terrain_object_reduced = {
                    let _span = tracing::info_span!(
                        "physics::granular_terrain_object_contacts",
                        substep,
                        iter
                    )
                    .entered();
                ...
                {
                    let _span =
                        tracing::info_span!("physics::granular_reduce_apply", substep, iter)
                            .entered();
                ...
                {
                    let _span =
                        tracing::info_span!("physics::granular_apply_positions", substep, iter)
                            .entered();
                    for i in 0..n {
                        if !particles.is_active_particle(i)
                            || is_water_particle(particles.material[i])
...
impl ParticleWorld {
    ...
    pub fn step_substeps(
        ...
        self.release_far_field_particles_for_active_chunks();
    ...
    fn step_single_substep(
        ...
        phase_cpu_start = process_cpu_time_seconds().unwrap_or(phase_cpu_end);
        {
            ...
            self.capture_far_field_intrusions(object_world);
...
fn world_pos_to_chunk(position: Vec2) -> IVec2 {
...
fn is_chunk_outside_radius(chunk: IVec2, center_chunk: IVec2, radius: i32) -> bool {
    let radius = radius.max(0);
    (chunk.x - center_chunk.x).abs() > radius || (chunk.y - center_chunk.y).abs() > radius
}

fn edge_sort_key(edge: ChunkEdge) -> i32 {
    match edge {
        ChunkEdge::North => 0,
        ChunkEdge::East => 1,
        ChunkEdge::South => 2,
...
mod tests {
    ...
    fn granular_solver_does_not_modify_water_only_particles() {
        ...
        let reaction = GranularSolver::solve_contacts(
            &mut particles,
            &terrain,
            &object_field,
            &object_world,
    ...
    fn granular_solver_pushes_particle_out_of_terrain() {
        ...
        let _ = GranularSolver::solve_contacts(
            &mut particles,
            &terrain,
            &object_field,
            &object_world,
