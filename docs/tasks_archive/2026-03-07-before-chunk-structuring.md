...
- 進捗:
  ...
  - 2026-03-05: 剛体系先送りに合わせて object 連成経路を縮退。`world/object.rs` を最小スタブ実装へ置換（重いSDF/広域場/分割ロジック削除）、`runtime/main/interface/scenario` から `ObjectPhysicsField` 依存を全廃。`cargo check --all-targets` 警告0件・`cargo test --lib` 全通過（29件）を確認。
  - 2026-03-05: object 生成/可視化経路を停止。`world_edit` の石ストローク object 化と break/delete 後 object 後処理を撤去し、`overlay/grid` の object 形状/姿勢描画を削除。`scenario` の object spawn は粒子生成のみ維持する形に変更。`cargo check --all-targets` 警告0件・`cargo test --lib` 全通過（29件）を確認。
  ...
  - 2026-03-05: `ContinuumParticleWorld` を撤去。GPU同期を `ParticleWorld <-> GPU` 直結へ変更（`sync.rs` で upload/readback を直接処理）、`physics/world/continuum.rs` と `world/mod.rs` export を削除。autoverify の granular diagnostics は GPU readback スナップショット参照へ切替。`cargo check --all-targets` 警告0件・`cargo test --lib` 全通過（28件）を確認。
  - 2026-03-05: `world_edit` の粒子編集を GPU 直行 request 化。`GpuWorldEditRequest` を Bevy `Message`（複数セル指定 + `AddParticles{material, count_per_cell}` / `RemoveParticles{max_count}`）として追加し、Water/Granular/Delete/Break の粒子変更をすべて message 発行へ移行。`apply_world_edit_requests` は `MessageReader` で upload 粒子列へ反映しつつ `ParticleWorld` を同順序で同期して readback index 整合を維持。`cargo check --all-targets` 警告0件・`cargo test --lib` 全通過（30件）を確認。
  - 2026-03-06: `Statistics` 仲介リソースを追加。`MpmStatisticsStatus`（total/phase/max_speed の enable/disable）と `MpmStatisticsSnapshot` を導入し、RenderWorld へ ExtractResource で転送。統計計算シェーダーを `assets/shaders/stats/` 配下へ分離（`clear_scalars` / `total_particles` / `phase_counts` / `max_speed`）。GPU node で有効項目のみ stats pass を実行し、`GpuStatisticsReadbackResult` 経由で CPU 側 snapshot へ反映。HUD と autoverify report の粒子カウント系は `MpmStatisticsSnapshot` 参照へ移行。`cargo check --all-targets` 警告0件・`cargo test --lib` 全通過（31件）・`cargo run -q -- --autoverify-config configs/autoverify/sand_water_interaction_drop.json` 実行確認（判定は既存閾値で fail=code6）。
  - 2026-03-06: テスト assertion/autoverify 向けに GPU 統計項目を拡張。`stats/penetration_tracking`, `water_surface_histogram/finalize`, `granular_repose_bounds/finalize`, `interaction_*`, `grid_density/finalize` を追加し、`MpmStatisticsStatus` で scenario ごとに必要項目のみ有効化する構成へ更新。`update_test_assert_panel` / `run_mpm_autoverify` は `ParticleWorld` 評価をやめ、`ScenarioStatisticsInput + evaluate_scenario_state_from_statistics` で GPU readback 統計を参照する形へ移行。`cargo check --all-targets`・`cargo test --lib` 通過、`cargo run -q -- --autoverify-config configs/autoverify/soil_repose_drop.json` 実行（終了 code=6, 既存閾値 fail）。
  - 2026-03-06: Save/Load を GPU 連携寄りへ更新。save は `MpmReadbackSnapshot`（全粒子 readback 経路）由来で保存し、load は `MpmGpuUploadRequest` を明示設定して clear+upload を強制するよう変更。あわせて `prepare_particle_upload` が明示 upload request を上書きしないよう修正し、`prepare_gpu_uploads` は空配列 upload も受理するよう変更。world edit から wake / particle fracture / drag velocity を削除。`cargo check --all-targets` / `cargo test --lib` 通過。
  - 2026-03-06: `ParticleWorld` 依存を追加整理。`gpu_mpm/sync` の world edit/readback から `ParticleWorld` ミラー同期を撤去し、編集は `MpmReadbackSnapshot + MpmGpuUploadRequest` 直結に変更。`prepare_gpu_params` の粒子数源も `ParticleWorld` ではなく upload/readback スナップショットへ変更。overlay UI/grid の未使用 `ParticleWorld` 参照を削除。さらに `save_load` に `SnapshotParticle` API（`save/load_*_with_particles`）を追加し、`runtime` と `main` の screenshot autoverify terrain snapshot 処理を GPU readback/upload ベースへ移行。あわせて no-op 化済みの `step_simulation_once` から `ParticleWorld` 引数を削除。`cargo check --all-targets` / `cargo test --lib` 通過。
  - 2026-03-06: Pure GPU化を継続。`runtime/fixed_update/physics::mod` から `ParticleWorld` resource 依存を撤去し、scenario/reset/save-load/replay は `SnapshotParticle -> MpmGpuUploadRequest` 直行へ統一。`scenario` の CPU粒子依存アサーション・artifact経路を整理して `write_scenario_artifacts_for_snapshot` ベースへ更新、`save_load` も `*_with_particles` API のみに整理。未使用化した `solver/step.rs` / `solver/types.rs` と `world/particle/mod.rs` を削除し、`ParticleMaterial` 参照を `physics::material` 直結へ切替。`cargo check --all-targets` / `cargo test --lib` 通過。
  - 2026-03-06: 追加整理として `FixedUpdate` 経路を撤去。`SimFixedSet` / `SimulationPerfMetrics` / `step_physics` / `finalize_frame_metrics` を削除し、初期ワールド構築のみを `runtime::initialize_default_world` に移動。`step_once` の1回実行保証は `prepare_gpu_run_state`（Update系）で consume する形へ移し、固定更新非依存で単発ステップ挙動を維持。`cargo check --all-targets` / `cargo test --lib` 通過。
  - 2026-03-06: さらに dead-path を整理。`SimulationParallelSettings` と UI の Parallel トグルを撤去、`PhysicsRegionSettings` / `PhysicsActiveRegion` を削除して overlay は GPU grid + updated chunk の表示に簡素化。更新元のなくなった `PhysicsStepProfiler` 一式（resource, UI panel, tooltip, profile segment型）も削除。`cargo check --all-targets` 警告0件・`cargo test --lib` 全通過（31件）を確認。
  - 2026-03-06: 追加入力系として terrain streaming の制御経路を整理。`TerrainStreamingSettings` と `stream_terrain_around_camera` system を削除し、カメラ追従アンロードの Update 経路を撤去。`physics/mod` と `solver/mod/runtime` の配線を追従更新。`cargo check --all-targets` 警告0件・`cargo test --lib` 全通過（31件）を確認。
  - 2026-03-06: 未使用APIを追加整理。`TerrainWorld` の `get_cell` / `for_each_in_chunk` / `gather_static_neighbors` と `solver/mpm_water.rs` の `is_mpm_managed_particle` を削除。あわせて不要化していた `#[allow(dead_code)]`（`MATERIAL_PATTERN_GRANULAR`, `get_loaded_cell_or_empty`, `world_to_cell`）を除去し、`render/terrain_gpu` の `world_cell_to_texture_index` は `#[cfg(test)]` 化。`cargo check --all-targets` / `cargo test --lib` 全通過（30件）を確認。
  - 2026-03-06: object系の残存パラメータを撤去。`SolverParams` の `object_*` フィールド群、`ParticlePhaseProperties` の `object_push_radius_m` / `object_repulsion_stiffness`、`params/material.rs` と `assets/params/material.ron` の `object_repulsion_stiffness`、`params/overlay.rs` と `assets/params/overlay.ron` の未使用 `grid.object_*` 設定を削除。`cargo check --all-targets` / `cargo test --lib` / `RUSTFLAGS='-Wunreachable-pub' cargo check --all-targets` を通過。
  - 2026-03-06: `params` の未参照経路を縮退。実行系から参照されていない `params/material` / `params/overlay` の Asset 定義・hot reload・active resource を `ParamsPlugin` から削除し、対応ファイル（`src/params/material.rs`, `src/params/overlay.rs`, `assets/params/material.ron`, `assets/params/overlay.ron`）を撤去。`physics/render/palette` の3系統のみ維持。`cargo check --all-targets` / `cargo test --lib` / `RUSTFLAGS='-Wunreachable-pub' cargo check --all-targets` を通過。
  - 2026-03-06: `physics/material` をGPU実利用の最小APIへ縮退。未参照だった `MaterialKind` / `MaterialForm` / `MaterialProperties` / `SolidBreakProperties`、および破壊関連API（`solid_break_properties`, `terrain_*break*` など）を削除し、`particle_properties` は `ParticleMaterial` 直マッチへ簡素化。`ParticlePhaseProperties` は実使用の `particles_per_cell` / `mass` / `radius_m` のみ保持。`cargo check --all-targets` / `cargo test --lib` / `RUSTFLAGS='-Wunreachable-pub' cargo check --all-targets` を通過。
  - 2026-03-06: `solver/params_types.rs` と `solver/params_defaults.rs` を撤去し、実使用されていた `fixed_dt` のみを `PhysicsParams`（`assets/params/physics.ron`）へ移設。`prepare_gpu_params` / `prepare_gpu_run_state` は `ActivePhysicsParams.fixed_dt` を参照するよう変更し、`PhysicsPlugin` の `SolverParams` resource 初期化を削除。シナリオ assertion の `active_after_seconds` 判定も引数 `fixed_dt` で評価する形へ更新（UI/autoverify 呼び出し追従）。`cargo check --all-targets` / `cargo test --lib` 通過。
  - 2026-03-06: 実行時調整パラメータの RON 化を拡張。`assets/params/overlay.ron` + `params::overlay` を復活し、Overlay UI 配置（ボタン位置・余白・フォント）と SDF overlay 描画スケール（step/radius/range/z）を hot reload 可能化。`overlay/ui.rs` は `apply_overlay_ui_params` で変更反映、`overlay/grid.rs` は `ActiveOverlayParams` 参照へ移行。あわせて `physics.ron` に runtime tuning（`boundary_velocity_sdf_threshold_h`, `stats_penetration_epsilon_m`, `max_substeps_per_frame`）を追加し、`gpu_mpm/sync` の該当ハードコードを置換。`cargo check --all-targets` / `cargo test --lib` 通過。
  - 2026-03-06: `camera_controller` / `interface` / `overlay` の残存UIパラメータを RON へ移設。`camera.ron` と `interface.ron` を追加し、`ParamsPlugin` に camera/interface/overlay の hot reload を再配線。`overlay/grid.rs` の chunk/更新色、`overlay/ui.rs` のトグル色を `ActiveOverlayParams` 参照へ統一。`interface` はボタン背景/枠線/HUD/ツールチップ/ダイアログ色、配置寸法、ツール挙動（ストローク間隔・break半径・FPS窓）を `ActiveInterfaceParams` 参照へ変更し、ツールアイコンの材質パレットも `interface.ron` から生成する形へ移行。`cargo check --all-targets` / `cargo test --lib` 通過（30件）。
  - 2026-03-06: `solver` ディレクトリを解体。`solver/mpm_water.rs` の phase 定義を `gpu_mpm/phase.rs` へ移設し、`main` / `gpu_mpm::sync` / 旧runtime参照を追従。`solver/runtime.rs` は `physics/runtime.rs` へ移して `PhysicsPlugin` の制御系（reset/save-load/replay）を直接参照する構成に変更。`src/physics/solver/` を削除し、`cargo fmt` / `cargo check --all-targets` / `cargo test --lib`（30件）を通過。
  - 2026-03-06: 命名整理として `physics/runtime.rs` を `physics/controls.rs` にリネーム。`physics/mod.rs` の `mod` 宣言と import を `controls` へ追従し、制御系モジュールの責務名を明確化。`cargo fmt` / `cargo check --all-targets` / `cargo test --lib`（30件）を通過。
- 完了条件:
