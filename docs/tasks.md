...
### [PHYS-03] Sub-blockマルチレート更新（固定dt + 境界インパルス負債）
...
- Subtasks:
  - [ ] `Sub-block` データ構造を `world` に追加する（更新レート、昇降格カウンタ、負債バッファ）。
  - [ ] `solver` にレートクラス別スケジューラを実装する（`rate=1/2/4`、最大比 `1:4`）。
  - [ ] レート境界拘束を実装する（高頻度側解法、低頻度側への反作用負債蓄積）。
  - [ ] 低頻度ブロック更新時の負債適用順を実装する（負債適用 -> 拘束反復）。
  - [ ] 強制昇格条件を実装する（負債閾値、侵入量、質量流束、接触継続時間）。
  - [ ] 侵入時の近傍伝播昇格を実装する（最小Active保持フレーム付き）。
  - [ ] 近距離領域では far-field 境界バッファを使わない分岐を実装する。
  - [ ] `render` のdirty反映を `Sub-block` 起点で局所化する。
  - [ ] `overlay` に更新レート可視化と負債ヒートマップを追加する。
  - [ ] `Tracy` 計測を追加し、`rate scheduler / debt accumulate / debt apply` の内訳を確認する。
  - [ ] 単体テストを追加する（昇格判定、負債適用順、境界条件）。
- 完了条件:
