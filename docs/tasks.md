...
- 連続LOD生成仕様の明文化が必要:
  - 生成ノイズは fBm を基本とし、フットプリント依存で有効帯域（寄与オクターブ）を制御する。
  - ベースノイズは差し替え可能にし、必要に応じて複数ノイズを合成できる前提を仕様化する。
- 遠近レンダリング規則の明文化が必要:
  - 近景は決定的サンプリング、遠景は期待値色を用い、中間帯は連続補間する。
  - 遠景でフレームごとランダム抽選は行わず、時間方向の安定性（ちらつき防止）を保証する。
- 材質確率場の明文化が必要:
  - `Stone/Soil/Empty` を確率場で定義し、合計確率の正規化と深度依存遷移関数を仕様に入れる。
  - 改変済みセル/チャンクは確率場より優先して反映する規則を明記する。
- LOD遷移の統計ブレンド規則を明文化したい:
  - 期待値色への遷移は固定距離閾値ではなく、`stderr / sqrt(N_eff)` ベースで決める。
  - `N_eff` の近似に使う相関長（cells）は調整可能パラメータとして扱う。
- 近景LOD維持規則の明文化が必要:
  - `span_chunks <= CELL_PIXEL_SIZE` では期待値ブレンドを使わず、従来ドットサンプリングを維持する。
