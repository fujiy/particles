...
### [MPM-WATER-01] 水向けMLS-MPM最小データ経路の構築
...
- Subtasks:
  - [ ] `ContinuumParticleWorld`（Water専用）を追加する。
  - [ ] `GridHierarchy` / `GridBlock` / `GridNode` の最小構造を追加する。
  - [ ] カーネル重みと勾配の共通APIを実装する（P2G/G2P共通）。
  - [ ] 決定論性（固定順序・固定しきい値）の単体テストを追加する。
- 完了条件:
...
- Subtasks:
  - [ ] P2G で質量・運動量・内部力寄与を実装する。
  - [ ] Grid Update で重力・内部力を適用してノード速度を更新する。
  - [ ] G2P で `v,x,C,F` を更新する。
  - [ ] `J=det(F)` の数値安定化クランプを実装する。
  - [ ] 単体テストを追加する（質量保存、静水安定、CFL判定）。
- 完了条件:
...
- スコープ:
  - 地形SDFを格子境界条件へ接続し、侵入抑制を実装する。
  - 連成ログ（運動量交換量）を取得できるようにする。
- Subtasks:
  - [ ] 地形SDFサンプラを MLS-MPM 格子更新段へ接続する。
  - [ ] 法線方向非貫通補正を実装する。
  - [ ] 接線減衰（境界摩擦）を実装する。
  - [ ] 侵入率メトリクスを追加する。
  - [ ] 地形接触シナリオの統合テストを追加する。
...
- Subtasks:
  - [ ] 粉体モデル候補（Drucker-Prager / frictional MPM など）を比較する。
  - [ ] 水-粉体相互作用の交換量定義を作成する。
  - [ ] 受け入れシナリオを定義する。
- 完了条件:
  - 粉体導入方針と試験仕様が文書化される。
