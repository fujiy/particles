...
## Todo
- [ ] 4近傍連結判定ユーティリティを実装（オブジェクト分割/地形分離で共通利用）
- [ ] オブジェクト破壊時に対象オブジェクトのみ連結成分分解を実行
- [ ] オブジェクト分割ロジックを実装（最大成分を既存 `ObjectId`、他成分を新規 `ObjectId`）
- [ ] 分割後のオブジェクト再構築を実装（粒子集合・`rest_local`・質量・描画/SDFキャッシュ）
- [ ] 破壊セルのみを除去し、無関係セルが同時破壊されないよう処理を修正
- [ ] `DETACH_FLOOD_FILL_MAX_CELLS` 定数を追加し、地形分離用の上限付きflood fillを実装
- [ ] 地形分離判定を実装（`count <= limit` はオブジェクト化、`count > limit` は地形保持）
- [ ] 地形→オブジェクト変換時の初期物理状態を実装（速度/角速度/姿勢の初期化）
- [ ] 分割・分離の受け入れテストを追加（オブジェクト部分破壊、地形小塊分離、地形大塊保持）
