...
- Subtasks:
  - [x] 粉体XPBDの拘束式で外力項（重力）を右辺へ取り込む実装方針を確定する。
  - [x] `scheduled` 粒子のみ更新する現行マルチレート経路と、外力統合後の時刻扱い（有効dt）を整合させる。
...
- 連続体ソルバ基盤の再選定が必要:
  - ADR-0001（`docs/adr-0001-mls-mpm-multirate-architecture.md`）に基づき、広域ワールド向け連続体基盤を MLS-MPM 中核へ移行する。
  - 空間 LoD + 時間 LoD（subcycling）と、剛体/地形との連成インターフェース仕様を実装タスクへ分解する。
