# Physics Report: Water + Granular MLS-MPM (Single Grid, GPU-First)

## 1. 目的

本書は、水と粉体（土/砂）を対象とした明示 MLS-MPM ソルバの実装仕様を、
...
### 2.3 粉体の構成則（Drucker-Prager）

土/砂の粉体は弾塑性MLS-MPM（frictional Drucker-Prager）で扱う。

- 弾性応力は線形等方弾性（`E`, `nu`）で計算する。
- 降伏判定は Drucker-Prager を用いる。
$$
f(\boldsymbol{\sigma}) = \sqrt{J_2} + \alpha I_1 - k \le 0
$$

ここで
- $I_1 = \mathrm{tr}(\boldsymbol{\sigma})$
- $J_2 = \frac{1}{2}\mathbf{s}:\mathbf{s}$（偏差応力不変量）
- $\alpha, k$ は `friction_angle_deg`, `cohesion_pa` から決まる係数

塑性更新は return mapping で行い、非連合流れを許可する。
$$
g(\boldsymbol{\sigma}) = \sqrt{J_2} + \beta I_1
$$

`beta` は `dilation_angle_deg` から決定する。

実装上の制約:
- 粉体の引張強度は持たない（引張側クランプ）。
- 土/砂の差はパラメータセットで表現し、構成則カーネルは共有する。
...
- material phase ID（`Water` / `GranularSoil` / `GranularSand`）
- 塑性体積指標 $J^p_p$（粉体のみ）
...
### 4.6 粉体塑性更新（return mapping）

粉体粒子では `G2P` 後に trial 応力を計算し、`f(\sigma)` を評価する。

- `f <= 0`: 弾性ステップ（塑性更新なし）
- `f > 0`: return mapping で降伏面へ射影し、`F_p` と $J^p_p$ を更新

このとき塑性散逸によりエネルギーは減少しうるが、運動量保存を優先する。
...
- 段階2: 粉体（Drucker-Prager）を同一MLS-MPM基盤へ統合
- 段階3: 剛体連成（GPU側へ境界速度場入力/反力出力I/Fを固定）

水-粉体連成（v1）の扱い:
- 同一ノード上で phase 別運動量を集計し、交換インパルスを対称更新する。
- 交換は「法線非貫通 + 接線摩擦/ドラッグ」で近似する。
- 液状化再現に必要な二相連成項は導入しない。
