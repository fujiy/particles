...
#### 4.4.1 地形SDFとの接触：非貫通 + クーロン摩擦（速度射影）
...
\tag{27}
$$
を用いる。 $\phi(\mathbf{x}_i)<\phi_{\mathrm{th}}$ のノードで、材料 $k$ ごとに以下を行う。
...
粉体粒子（$k(p)=\mathrm{g}$）では Eq. (34) のあとに
...
\tag{44}
$$
$\mathbf{J}=\mathbf{J}_n+\mathbf{J}_t$ を Eq. (40) と同様に対称適用する。
