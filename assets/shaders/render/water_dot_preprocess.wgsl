...
@group(0) @binding(2) var<storage, read_write> density_atomic: array<atomic<u32>>;
@group(0) @binding(3) var<storage, read_write> blur_tmp: array<f32>;
@group(0) @binding(4) var<storage, read_write> blurred_density: array<f32>;
...
fn clear_density(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    atomicStore(&density_atomic[idx], 0u);
    blur_tmp[idx] = 0.0;
    blurred_density[idx] = 0.0;
}
...
fn splat_particles(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var y = min_y; y <= max_y; y = y + 1) {
        for (var x = min_x; x <= max_x; x = x + 1) {
            ...
            atomicAdd(&density_atomic[dot_index(x, y)], atomic_units);
        }
    }
}
...
fn blur_x(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var k = -radius; k <= radius; k = k + 1) {
        ...
        let sample = f32(atomicLoad(&density_atomic[dot_index(sx, y)])) / params.atomic_scale;
...
fn blur_y(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    blurred_density[idx] = density_sum / max(weight_sum, 1.0e-6);
}
